...
! ***********************************************************************
      ...
      subroutine show_net_reactions_and_info(handle, iounit, ierr)
         ...
         if (ierr /= 0) return
         write(iounit,*)
         write(iounit,'(4x,a30,3a16,4x,a4)') 'name', 'Qtotal', 'Qneu', 'category', 'info'
         do i = 1, g% num_reactions
            id = g% reaction_id(i)
...
!=============changed, added neu_reac and radius and age args============         
      subroutine net_get( &
            handle, neu_reac_info, just_dxdt, neu_reac, n, num_isos, &
...
!=========changed below, added neu_reac and radius and age args=========
         call eval_net( &
               neu_reac_info, n, g, rates_only, just_dxdt, neu_reac, &
               num_isos, num_reactions, &
               g% num_wk_reactions, &
...
!=====Changed below, added zero array and first false as dummy args====
         ...
         call eval_net( &
               neu_reac_info, n, g, rates_only, just_dxdt, .false., &
...
!=====Changed below, added zero array and first false as dummy args===
      ...
      subroutine net_1_zone_burn( &
         ...
         real(dp), intent(inout), pointer :: ending_x(:) ! (num_isos)
      ...
      subroutine net_1_zone_burn_const_density( &
         ...
         call burn_const_density_1_zone( &
            ...
            use_pivoting, trace, dbg, burner_finish_substep, &
            burn_lwork, burn_work_array, net_lwork, net_work_array, &
            ending_x, ending_log10T, avg_eps_nuc, ending_eps_neu_total, &
            nfcn, njac, nstep, naccpt, nrejct, ierr)
