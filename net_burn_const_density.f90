...
! ***********************************************************************
      ...
      use chem_def
      use net_def
      use rates_def, only: num_rvs, ir1212, ir_he4_he4_he4_to_c12, &
         use_rate_1212_G05, use_rate_3a_FL87
      ...
      subroutine burn_const_density_1_zone( &
            ...
            ending_x, ending_log10T, avg_eps_nuc, eps_neu_total, &
         ...
         real(dp), intent(inout), pointer :: ending_x(:) ! (species)
         ...
         real(dp) :: xh, xhe, z, abar, zbar, z2bar, ye, mass_correction, sumx
         ...
         !if (stptry == 0) then
            stptry = 1d-16 ! TESTING       max((stopp - start) * 1.0d-10,1.0d-12)
...
!======================            
            ...
            do i=1,species
               if (is_bad(y(i))) then
                  write(*,2) 'failed in jakob_or_derivs: bad y(i)', i, y(i)
                  stop
               end if               
...
!=======Changed below, added zero array and first false as dummy args===
            ...
            call eval_net( &
               neu_reac_info, n, g, rates_only, dxdt_only, .false., &
