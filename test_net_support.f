...
!$OMP END PARALLEL DO
      ...
      subroutine do_one_net_eval(species, num_reactions, logT, T, logRho, Rho,  &
         ...
         real(dp) :: z, abar, zbar, z2bar, ye, sum, mx, weak_rate_factor
...
!==================
         ...
         call composition_info( &
            species, chem_id, xin, xh, xhe, z, abar, zbar, z2bar, ye,  &
...
!=======Changed below, added zero array and first false as dummy args=== 
         do w=1, 4
            neu_reac_info(w) = 0d0
         end do
         call net_get(handle, neu_reac_info, skip_jacobian, .false., n, &
                  species, num_reactions,  &
 ...
 ! the data files have columns of mass, radius, density, temp
      ...
      subroutine Do_One_Testcase(net_file, do_timing, show_Qs)
         ...
         integer :: i, j, k, info, ierr, times_total, elapsed_time, clock_rate, time0, time1
         ...
         logical :: skip_jacobian, doing_d_dlnd
 ...
 ! co55 -> fe55
         ...
         !else if (net_file == 'pp_extras.net') then
         else if (net_file == 'pp_extras_alternate.net') then
            
            nrates_to_show = 1
            
            if (rates_reaction_id('r_h1_li7_to_he4_he4') <= 0) stop 'bad reaction'
            write(*,*) 'r_h1_li7_to_he4_he4', rates_reaction_id('r_h1_li7_to_he4_he4')
         
            rates_to_show(1:nrates_to_show) = (/  &
            rates_reaction_id('r_h1_li7_to_he4_he4') /)
     
                                    xin(net_iso(ih1))=     7.1747643111461012D-01
                                   xin(net_iso(ihe4))=     2.6749682381518713D-01
                                   xin(net_iso(io16))=     6.4313512675752381D-03
                                  xin(net_iso(img24))=     3.7285925101300022D-03
                                   xin(net_iso(ic12))=     2.6531853024828449D-03
                                  xin(net_iso(ine20))=     1.4096571622722735D-03
                                   xin(net_iso(in14))=     7.7720339152726110D-04
                                   xin(net_iso(ihe3))=     2.6750342020458006D-05
                                   xin(net_iso(ili7))=     5.0941947429457045D-09
                                    xin(net_iso(ih2))=     1.7192690779086208D-17
                                   xin(net_iso(ibe7))=     3.8388215117010835D-26
                                    xin(net_iso(ib8))=     2.8989148482611920D-53
                              
                              write(*,*) 'sum xin', sum(xin(:))

                                  logT =    6.3462379741492159D+00
                                logRho =    -2.0196753306560386D-01
                                   eta =   -3.9340249245963053D+00
         ...
         xin = 0
            ...
            /)
                     xin = 0
                                   ...
                                   xin(net_iso(ihe4))=     9.9025885531762803D-01
                                   xin(net_iso(in14))=     6.3528138472163037D-03
                                  xin(net_iso(img24))=     1.4475457678246463D-03
                                  xin(net_iso(ine20))=     7.4298861270028307D-04
                                  xin(net_iso(ife56))=     4.3562378387923930D-04
                                  xin(net_iso(isi28))=     2.4342519862312340D-04
                                   xin(net_iso(is32))=     1.4754770656960669D-04
                                   xin(net_iso(ic12))=     1.3107507701151931D-04
                                   xin(net_iso(io16))=     1.1558330621623311D-04
                                    xin(net_iso(ih1))=     4.6780337523994345D-05
                                  xin(net_iso(iar36))=     2.8853458942170840D-05
                                  xin(net_iso(ife54))=     2.6579164311461265D-05
                                  xin(net_iso(ica40))=     2.2328421535853810D-05
                                   xin(net_iso(ihe3))=     5.3899365589370187D-17
                                  xin(net_iso(iti44))=     1.3201357271010226D-56
                                  xin(net_iso(icr48))=     1.0004437560504204D-99
                                  xin(net_iso(iprot))=     1.0004437560503016D-99
                                  xin(net_iso(ineut))=     1.0004437560503016D-99
                                  xin(net_iso(icr56))=     1.0004437560503016D-99
                                  xin(net_iso(ife52))=     1.0004437560503016D-99
                                  xin(net_iso(ico56))=     0.0000000000000000D+00
                                  xin(net_iso(ini56))=     0.0000000000000000D+00
                              write(*,*) 'test case sum xin', sum(xin(1:species))
                              

                                                 logT =    7.5614101623429990D+00
                                               logRho =   -1.4466714835712152D-01
                                                    T =    10**logT
                                                  rho =    10**logRho
                                                 abar =    4.0289469715442117D+00
                                                 zbar =    2.0145030581209018D+00
                                                z2bar =    4.1689726882805482D+00
                                                   ye =    5.0000733996972535D-01
                                                  eta =   -8.5076591998144977D+00
                        screening_mode = extended_screening
                                              theta_e =    0.0000000000000000D+00
         ...
         if (.false.) then ! dfridr tests for partials
            ...
            if (doing_d_dlnd) then
               ...
               dvardx_0 = d_dxdt_dRho(1)*Rho ! analytic value of partial
            else
               dx_0 = max(1d-14, abs(logT*ln10*1d-6))
               dvardx_0 = d_dxdt_dT(1)*T ! analytic value of partial
            end if
            err = 0d0
